...
des_IR_edge_path = 'Your txt result storage directory';
...
if ~exist(des_IR_edge_path, 'dir'), mkdir(des_IR_edge_path);end
...
for k = 2: 2
    ...
    fid = fopen(fullfile(des_IR_edge_path, txt_file_name), 'a+');
    while fid ==-1
        fid = fopen(fullfile(des_IR_edge_path, txt_file_name), 'a+');
    end
    ...
    for j = 1:99
        ...
        for i = 1:length(day_file_names)
            ...
            IR_edge = edge(rgb2gray(IR_img), 'canny', [low_th, high_th]);
...
fprintf('AP under different threshold is %.3f\n', mean_AP);