...
class CyTranModel(BaseModel):
    ...
    def HU_loss(self, X, G_X, Y, alpha_HU):
        # Calculate the absolute difference in HU values between the generated image and the native arterial image
        diff_G_X = torch.abs(G_X - X)

        # Calculate the absolute difference in HU values between the generated image and the native venous image
        diff_G_Y = torch.abs(G_X - Y)

        # HU_loss = torch.mean(diff_G_X + torch.abs(1 - diff_G_Y)) * alpha_HU
        # HU_loss = torch.mean(diff_G_X) - torch.mean(diff_G_Y) * alpha_HU
        # HU_loss = (1 - torch.mean(diff_G_X)) * torch.mean(diff_G_Y) * alpha_HU
        # HU_loss = (1 - torch.mean(diff_G_X)) * torch.mean(diff_G_Y) * alpha_HU
        HU_loss = torch.mean(diff_G_X + (2 - diff_G_Y)) * alpha_HU
    ...
    def backward_G(self):
        ...
        # HU loss
        self.hu_loss = self.HU_loss(self.real_B, self.fake_B, self.real_A, 10)
        
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.hu_loss
