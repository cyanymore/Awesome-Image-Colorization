...
parser.add_argument("--x_test_data_path", default='D:/cccc/cartoon1/test_picture1/', help="path of x test datas.") #x域的测试图片路径
#parser.add_argument("--y_test_data_path", default='D:/cccc/cartoon1/test_label1/', help="path of y test datas.") #y域的测试图片路径
parser.add_argument("--image_size", type=int, default=256, help="load image size") #网络输入的尺度
parser.add_argument("--snapshots", default='D:/cccc/snapshots_newcartoon/',help="Path of Snapshots") #读取训练好的模型参数的路径
parser.add_argument("--out_dir_x", default='D:/cccc/test_output_x__newcartoon1/',help="Output Folder") #保存x域的输入图片与生成的y域图片的路径
parser.add_argument("--out_dir_y", default='D:/cccc/test_output_y__newcartoon1/',help="Output Folder") #保存y域的输入图片与生成的x域图片的路径
...
def get_write_picture(x_image,  fake_y): #get_write_picture函数得到网络测试结果
...
def main():
    ...
    for step in range(total_step):
        ...
        x_write_image = get_write_picture(test_ximage, fake_y_value) #得到最终的图片结果
