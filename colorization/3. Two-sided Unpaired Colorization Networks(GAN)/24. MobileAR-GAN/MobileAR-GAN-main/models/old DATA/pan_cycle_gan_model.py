...
class PanCycleGANModel(BaseModel):
    def name(self):
        return 'PanCycleGANModel'
...
#            self.netFeat = networks.define_F(self.gpu_ids, use_bn=False).to(self.device)
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        real_inters = netD.get_intermediate_outputs()
#        print(real_inters)
        ...
        loss_D_fake = self.criterionGAN(pred_fake, False)
         # outputs of intermediate layers
        fake_inters = netD.get_intermediate_outputs()
#        print(fake_inters)
        ...
        if self.loss_PAN.data.item() > self.pan_mergin_m:
        ...
        # Combined loss
#        print('loss_PAN->%f and self.loss_PAN->%f' %(loss_PAN, self.loss_PAN))
        loss_D = (loss_D_fake + loss_D_real) * 0.5 + loss_PAN
#        print(loss_D)
...
#        self.feat_loss_BrecB = self.criterionFeat(self.netFeat(self.real_B), self.netFeat(self.rec_B)) * lambda_feat_BrecB 
        ...
        # combined loss
        self.loss_G = self.loss_G_A + self.loss_G_B  + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.feat_loss +self.loss_SynA + self.loss_SynB + self.loss_CSA + self.loss_CSB + self.loss_PAN
