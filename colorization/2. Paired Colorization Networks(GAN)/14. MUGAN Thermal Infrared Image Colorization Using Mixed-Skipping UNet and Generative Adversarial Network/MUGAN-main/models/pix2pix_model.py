...
class Pix2PixModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            if opt.which_model_netD == 'multi':
                ...
                self.criterionGAN = networks.GANLoss(use_lsgan=opt.no_lsgan).to(self.device)  #原本是use_lsgan=not opt.no_lsgan
    ...
    def backward_G(self):
        ...
        self.loss_G = self.loss_G_GAN * self.w_gan + self.loss_G_L1 + self.loss_vgg * self.w_vgg + self.tv_loss * self.w_tv + self.w_ss * self.loss_ssim
