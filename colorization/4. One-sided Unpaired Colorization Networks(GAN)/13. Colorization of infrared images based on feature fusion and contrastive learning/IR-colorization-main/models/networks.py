...
from . import IRcolorization_networks
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    :return:
    """
    norm_value = IRcolorization_networks.get_norm_layer(norm)

    if netG == 'resnet_9blocks':
        net = IRcolorization_networks.ResnetGenerator(input_nc, output_nc, ngf, norm_value, use_dropout, n_blocks=9, no_antialias=no_antialias, no_antialias_up=no_antialias_up, opt=opt)

    elif netG=='UNet_2Plus':
        net=IRcolorization_networks.UNetnest(input_nc,output_nc)
    elif netG == 'unet_256':
        net = IRcolorization_networks.UnetGenerator(input_nc, output_nc,num_downs=8)
    elif netG==' UNet_2Plus':
        net=IRcolorization_networks.UNetnest(input_nc,output_nc)
    # /
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % netG)
    return IRcolorization_networks.init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netG))
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    norm_value = IRcolorization_networks.get_norm_layer(norm)
    if netD == 'basic':
        net = IRcolorization_networks.NLayerDiscriminator(input_nc, ndf, n_layers_D, norm_value, no_antialias)
    elif netD == 'bimulti':
        net = IRcolorization_networks.D_NLayersMulti(input_nc, ndf, n_layers=n_layers_D, norm_layer=norm_value, num_D=2)

    else:
        raise NotImplementedError('Discriminator model name [%s] is not recognized' % netD)
    return IRcolorization_networks.init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netD))
