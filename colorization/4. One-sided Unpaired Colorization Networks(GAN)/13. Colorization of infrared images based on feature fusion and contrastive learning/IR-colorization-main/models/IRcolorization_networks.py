...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[], debug=False, initialize_weights=True):
    ...
    return net
class UNetnest(nn.Module):

    def __init__(self, input_nc=1, output_nc=3, feature_scale=4, is_deconv=True, is_batchnorm=True, is_ds=True):
        super(UNetnest, self).__init__()
        ...
        # downsampling
        self.conv00 = unetConv2(self.input_nc, filters[0], self.is_batchnorm)
        ...
        self.final_1 = nn.Conv2d(filters[0], output_nc, 1)
        self.final_2 = nn.Conv2d(filters[0], output_nc, 1)
        self.final_3 = nn.Conv2d(filters[0], output_nc, 1)
        self.final_4 = nn.Conv2d(filters[0], output_nc, 1)
    ...
    def forward(self, x, layers=[], encode_only=False):
        ...
        if len(layers) > 0:
            feat = x
            feats = []
            for layer_id, layer in enumerate(X_00,X_10,X_20,X_30,X_40,X_31,X_22,X_13,X_04):
                feat = layer(feat)
                if layer_id in layers:
                    feats.append(feat)
                if layer_id == layers[-1] and encode_only:
