...
class COLORModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_spatial', type=float, default=10.0, help='weight for loss')
        parser.add_argument('--lambda_spatial_idt', type=float, default=0, help='weight for idt spatial loss')
        parser.add_argument('--lambda_perceptual', type=float, default=1.0, help='weight for feature consistency loss')
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['style', 'G_s', 'per', 'D_real', 'D_fake', 'G_GAN', 'tv']
        ...
        if self.isTrain:
            ...
            self.criterionSpatial = losses.SpatialCorrelativeLoss(opt.loss_mode, opt.patch_nums, opt.patch_size,
                                                                  opt.use_norm,
                                                                  opt.learned_attn, gpu_ids=self.gpu_ids, T=opt.T).to(
    ...
    def data_dependent_initialize(self, data):
        ...
        if self.isTrain:
            ...
            if self.opt.learned_attn:
                self.optimizer_F = torch.optim.Adam([{'params': list(
                    filter(lambda p: p.requires_grad, self.netPre.parameters())), 'lr': self.opt.lr * 0.0},
                                                     {'params': list(
                                                         filter(lambda p: p.requires_grad, self.netF.parameters()))}],
    ...
    def forward(self):
        """Run forward pass"""
        self.real = torch.cat((self.real_A, self.real_B), dim=0) if (
                                                                                self.opt.lambda_identity + self.opt.lambda_spatial_idt > 0) and self.opt.isTrain else self.real_A
    ...
    def backward_F(self):
        ...
        norm_real_A, norm_real_B, norm_fake_B = self.normalization((self.real_A + 1) * 0.5), self.normalization(
            (self.real_B + 1) * 0.5), self.normalization((self.fake_B.detach() + 1) * 0.5)
        if self.opt.augment:
            norm_aug_A, norm_aug_B = self.normalization((self.aug_A + 1) * 0.5), self.normalization(
                (self.aug_B + 1) * 0.5)
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        if self.opt.lambda_gradient > 0.0:
            self.loss_D_Gradient, _ = losses.cal_gradient_penalty(netD, real, fake, real.device,
                                                                  lambda_gp=self.opt.lambda_gradient)  #
    ...
    def backward_G(self):
        ...
        self.loss_tv = (diff_i + diff_j) / (256 * 256)
        self.loss_G_L1 = self.criterionIdt(self.fake_B, self.real_B)
        # identity loss
        if l_spatial_idt > 0:
        ...
        self.loss_G = self.loss_G_GAN + self.loss_style + self.loss_per + self.loss_G_s + self.loss_G_s_idt_B + self.loss_idt_B + self.loss_tv
