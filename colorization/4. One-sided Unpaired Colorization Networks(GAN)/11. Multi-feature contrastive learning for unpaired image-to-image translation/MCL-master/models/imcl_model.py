...
class IMCLModel(BaseModel):
    """ This class implements MCL and FastMCL model, described in the paper
    ...
    def modify_commandline_options(parser, is_train=True):
        """  Configures options specific for MCL model
        """
        parser.add_argument('--IMCL_mode', type=str, default="IMCL", choices='(IMCL, imcl, FastIMCL, fastimcl)')
        ...
        parser.add_argument('--lbd', default=0.01, type=float,
                            help='Relative strength of the fake loss of sup and simclr')
        parser.set_defaults(pool_size=0)  # no image pooling

        opt, _ = parser.parse_known_args()

        # Set default parameters for MCL and FastMCL
        if opt.IMCL_mode.lower() == "imcl":
            parser.set_defaults(nce_idt=True, lambda_NCE=1.0)
        elif opt.IMCL_mode.lower() == "fastimcl":
            parser.set_defaults(
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE' , 'mcl']
    ...
    def compute_D_loss(self):
        ...
        self.loss_D_fake = self.criterionGAN(pred_fake, False).mean() * self.opt.lambda_GAN
        # Real
        self.pred_real = self.netD(self.real_B)
        loss_D_real = self.criterionGAN(self.pred_real, True)
        self.loss_D_real = loss_D_real.mean() * self.opt.lambda_GAN

        #loss_mcl
        mcl_fake = F.normalize(pred_fake.view(-1, 30))
        mcl_real = F.normalize(self.pred_real.view(-1, 30))
        self.loss_mcl = self.mcl_fake(mcl_fake, mcl_real, temperature = self.opt.temp)

        # combine loss and calculate gradients
        self.loss_D = (self.loss_D_fake + self.loss_D_real) * 0.5 + self.loss_mcl * self.opt.lbd
        return self.loss_D
