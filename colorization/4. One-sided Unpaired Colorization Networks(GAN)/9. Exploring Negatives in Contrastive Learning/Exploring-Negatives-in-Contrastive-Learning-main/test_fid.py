...
if __name__ == '__main__':
    ...
    for epoch in epoches:
        ...
        if opt.aspect_ratio == 2 :
            #paths = [os.path.join(web_dir, 'images', 'fake_B'), os.path.join(opt.dataroot, 'result2')]
            paths = [os.path.join(web_dir, 'images', 'fake_B'), os.path.join(web_dir, 'images',  'real_B')]       
        else:
            paths = [os.path.join(web_dir, 'images', 'fake_B'), os.path.join(opt.dataroot, 'testB')]     
    ...
    df['epoch'] = x
    df['fid'] =y
    df.to_csv(os.path.join(opt.results_dir, opt.name,opt.name+'.csv'),index=0)
